      ...
      <div className="relative">
        ...
        >
          ...
          {modulatedPercent !== undefined && (
            <div
              className="absolute w-2 h-2 bg-blue-400/50 rounded-full -translate-x-1/2 pointer-events-none"
              style={{ left: `${modulatedPercent}%` }}
            />
          )}

          <SliderPrimitive.Thumb className="block w-4 h-4 bg-white border-2 border-blue-500 rounded-full hover:bg-blue-50 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 disabled:cursor-not-allowed" />
        </SliderPrimitive.Root>
      </div>
      {valueDisplay && (
        <span className="text-sm text-gray-600 dark:text-gray-400">{valueDisplay}</span>
      )}
    </label>
  );
}
